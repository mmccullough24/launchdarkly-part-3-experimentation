...
# 2. The estimator is unbiased — measured rates should converge on --control-rate
#    and control-rate*(1+lift). Anything more than ~1 SE off at this size is a bug,
#    not noise.
...
- **Per-visitor RNG, seeded from the context key.** `_visitor_rng()` deliberately replaces a single
  ...
  bug, caught by comparing a 60,000-visitor run against ground truth.
...
it is what makes the demo credible rather than a mock.
